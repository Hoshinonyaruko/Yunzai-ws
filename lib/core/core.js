        ...
        setInterval(()=>{
            const json = JSON.stringify({
                ...
                status: {
                    ...
                    "plugins_good": null,
                    "stat": {
                        "packet_received": 10771,
                        "packet_sent": 9759,
                        "packet_lost": 0,
                        "message_received": 613,
                        "message_sent": 288,
...
export function dipatch(event) {
    if (!assert(event))
        return;
    var cache = [];
    var json = JSON.stringify(event, function(key, value) {
...
websockets.forEach((ws)=>{
    ...
    var tmp_json = JSON.parse(json)
    tmp_json.message = tmp_json.raw_message
    if(typeof tmp_json.message_id === 'string')
    tmp_json.message_id = hashString(tmp_json.message_id)
    tmp_json.message_id = tmp_json.message_id
    tmp_json.font = 123
    tmp_json.message_seq = message_seq
    json = JSON.stringify(tmp_json)
    console.log(`反向WS(${ws.url})上报事件: ` + json)
    last_message_time = parseInt(Date.now()/1000)
    ws.send(json);
});
...
async function onWSMessage(ws, data) {
    ...
    try {
        ...
        if (
            ...
            data.message_id = hashString(data.message_id);
            const {echo, ...newdata} = data;
            let newRet = {
                retcode: 0,
                status: "ok",
                data: {},
            ...
            newRet.data = Object.assign(newRet.data, newdata);
            ret = JSON.stringify(newRet);
            console.log('反向ws连接(${url}):信息发送成功回执:'+ret)
        }
        ws.send(ret);
    } catch (e) {
            var retcode = 1400;
